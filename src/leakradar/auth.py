...
class LicenseManager:
    ...
    Validates license keys directly against Dodo Payments official production API (zero custom cloud server needed).
    """
    ...
    async def activate_license(cls, license_key: str) -> LicenseContext:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                # Query Dodo Payments Production API validation & activation endpoints
                for url in [cls.DODO_LIVE_URL, "https://live.dodopayments.com/licenses/activate"]:
                    try:
                        resp = await client.post(url, json={"license_key": clean_key, "name": fingerprint})
        ...
        # If Dodo Payments Production validation fails, fall back strictly to free tier
        return LicenseContext(active=False, tier="free")
