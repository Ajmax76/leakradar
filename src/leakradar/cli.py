...
def check_github_release_update():
    """
    Queries GitHub Releases API (1.5s timeout) to notify the user if a newer version is available.
    """
    try:
        url = "https://api.github.com/repos/Ajmax76/leakradar/releases/latest"
        with httpx.Client(timeout=1.5) as client:
            resp = client.get(url, headers={"User-Agent": f"leakradar-cli/{__version__}"})
            if resp.status_code == 200:
                ...
                if tag and tag != __version__:
                    console.print(
                        Panel(
                            f"[bold yellow]🔔 Update Available: v{tag}[/bold yellow] (Current: v{__version__})\n\n"
                            f"Download latest release binaries for Windows, Linux & macOS:\n"
                            f"[bold cyan]https://github.com/Ajmax76/leakradar/releases/latest[/bold cyan]",
                            title="[bold green]LeakRadar Update Notice[/bold green]",
                            border_style="yellow",
                            expand=False,
                        )
                    )
    ...
    if format_choice.lower() in ("pdf", "all"):
        if not lic_ctx.active:
            ...
            if format_choice.lower() == "pdf":
                raise typer.Exit(code=1)
        elif PDFReportGenerator is None:
            console.print("[bold red]PDF export module requires reportlab. Install via `pip install leakradar-cli[pdf]`.[/bold red]")
            if format_choice.lower() == "pdf":
                raise typer.Exit(code=1)
        else:
